...
typedef enum {
	...
	VEHICLE_STATE_BATTERY_CHARGE = 8,
} vehicle_state_t;
...
#define CAN_ID_BMS_status_1 (0x2000003)
...
#define CAN_ID_BMS_status_2 (0x2000004)
...
#define CAN_PERIOD_charger_status (0)
typedef struct __attribute__((__packed__)) {
	...
	uint8_t pad_byte2;
	uint8_t pad_byte3;
} CAN_MSG_charger_status_T;


// Messages from node (4): laptop
#define CAN_ID_charger_control (0x1806e5f4)
#define CAN_PERIOD_charger_control (0)
typedef struct __attribute__((__packed__)) {
	...
	uint8_t pad_byte2;
	uint8_t pad_byte3;
} CAN_MSG_charger_control_T;
typedef enum {
	CHARGER_ENABLED_charge = 0,
	CHARGER_ENABLED_no_charge = 1,
} charger_enabled_t;
